...
import PlusIcon from "../icons/ICN_Plus.svg?react";
...
const EpisodeItem = (props) => {
  ...
  const recensioni = props.giochi.filter(
    (gioco) => gioco.tipologia === "Recensione",
  );
  const chiacchiere = props.giochi.filter(
    (gioco) => gioco.tipologia === "Chiacchiera libera",
  );
  const consigli = props.giochi.filter(
    (gioco) => gioco.tipologia === "Consiglio",
  );
    ...
    <>
      <li className={classes.cardBox}>
        ...
        >
          <img
            ...
            src={props.cover}
            alt={`cover dell'episodio ${props.numero}`}
          />
          <div className={classes.textBox}>
            ...
            <div className={classes.additionalInfo}>
              <p className={classes.releaseDate}>
                {props.uscita.replaceAll("-", ".")}{" "}
              </p>
              <p>&middot;</p>
              <p className={classes.duration}>
                {(props.durata / 60).toFixed(0)} min.
              </p>
            </div>
      ...
      {props.active ? (
        <li
          ...
          style={
            !isMobile
              ? { gridRowStart: `${Math.floor(props.index / 2 + 2)}` }
              : {}
          }
        >
          <EpisodeContent
