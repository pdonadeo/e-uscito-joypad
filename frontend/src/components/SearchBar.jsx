...
import SearchIcon from "../icons/ICN_Search.svg?react";
import SortIcon from "../icons/ICN_Sort.svg?react";
import CloseIcon from "../icons/ICN_Close.svg?react";
...
const SearchBar = () => {
  ...
  const boldName = (name, search) => {
    ...
    if (isMatch) {
      ...
      if (index !== 0) {
        return (
          <>
            {name.substr(0, index)}
            <strong>{name.substr(index, search.length)}</strong>
            {name.substr(index + search.length, name.length)}
          </>
        );
      }
    }
    if (name.substr(0, search.length).toLowerCase() === search.toLowerCase()) {
      return (
        <>
          <strong>{name.substr(0, search.length)}</strong>
          {name.substr(search.length)}
        </>
      );
    }
    return name;
      ...
      <div className={classes.searchBar}>
        ...
        {searchCtx.searchInput.trim() === "" && searchWords === "" ? (
          <>
            ...
            <div
              className={`${classes.sortModal} ${showModal ? classes.show : ""}`}
            >
              <p className={classes.sortText}>ORDINA PER</p>
              <div className={classes.sortContainer}>
                <p
                  className={classes.sortControl}
                  onClick={reverseHandler.bind(this, "ascending")}
                >
                  Più recenti
                </p>
                <div
                  className={
                    sorting === "ascending" ? classes.showCheckIcon : ""
                  }
                >
                  {checkIcon}
                </div>
              ...
              <div className={classes.sortContainer}>
                <p
                  className={classes.sortControl}
                  onClick={reverseHandler.bind(this, "descending")}
                >
                  Meno recenti
                </p>
                <div
                  className={
                    sorting === "descending" ? classes.showCheckIcon : ""
                  }
                >
                  {checkIcon}
                </div>
