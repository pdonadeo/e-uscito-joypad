...
let get_last_episods db ?(n = 10) ~offset () =
  ...
  let q =
    ...
    ->* t12
          ...
          (option string)
          (option string)
          (t3 (option string) (option string) (option string)))
      {|
          SELECT
            id,
            to_char(ts_created, 'YYYY-MM-DD HH24:MI:SS.USTZH:TZM') AS ts_created,
            to_char(ts, 'YYYY-MM-DD HH24:MI:SS.USTZH:TZM') AS ts,
            titolo,
            episodio_numero,
            data_uscita,
            durata,
  ...
  let* records = DB.collect_list q (n, offset) in
  let records =
    ListLabels.map records ~f:(fun r ->
        let ( id,
              ...
              url_video,
              url_post,
              (note, descrizione_html, descrizione_txt) ) =
          r
        in
        {
          id;
    ...
    then begin
      ...
      let q =
        ...
        ->* t12
              ...
              (option string)
              (option string)
              (t5 (option string) (option string) (option string) float int))
          {|
                SELECT
                  id,
                  ts_created,
                ...
                FROM (
                    ...
                    WHERE unaccent(game.titolo) ILIKE unaccent(?)
          ...
          {|
                ...
                FROM (
                    SELECT DISTINCT ON (ep.id)
                      ...
                      ep.descrizione_html,
                      ep.descrizione_txt,
                      word_similarity(unaccent(?), unaccent(game.titolo)) AS similarity,
                      (
                        CASE
                          WHEN ass.tipologia = 'RECE' THEN 1
                          WHEN ass.tipologia = 'CONS' THEN 2
  ...
  let records =
    ListLabels.map records ~f:(fun r ->
        let ( id,
              ...
              url_video,
              url_post,
              (note, descrizione_html, descrizione_txt, _similarity, _priority) ) =
          r
        in
        {
          id;
...
let search_episodes_by_game_id db ~game_id () =
  ...
  in
  let module DB = (val db : Caqti_lwt.CONNECTION) in
  let* records = DB.collect_list q game_id in
  let records =
    ListLabels.map records ~f:(fun r ->
        let ( id,
              ts_created,
